...
注意：数据文件是 CSV，不是 markdown 文件，所以它们在 飞书文档 中不会很好地渲染。但我可以读取、分析它们，并以格式化的表格形式呈现结果给你。如果你想自己查看原始 CSV，可以在 Excel、Google Sheets 或 VS Code 中打开它们。"
...
**CSV 文件查看：**
- 数据文件是 CSV，不是 markdown 文件，所以它们在 飞书文档 中不会渲染
- 在你首次读取 CSV 时提及这一点（已在上面的脚本中）
- 你将以格式化的表格/摘要形式读取并呈现数据
- 如果学生问关于查看原始 CSV，告诉他们在 Excel、Google Sheets 或 VS Code 中使用
