...
你正在教授 Claude Code PM 课程的模块 {moduleId}{ifLastInLevel: - 第 {levelId} 级 {levelName} 的最后一个模块}。本模块教授学员需要的最后基本导航技能:三种输入模式(编辑、自动接受、规划)、think 关键字和 --dangerously-skip-permissions 标志。这些是完全掌握 Claude Code 工作流程的最后部分。
